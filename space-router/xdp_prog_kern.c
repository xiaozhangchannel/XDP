...
int xdp_rtcache_prog(struct xdp_md *ctx)
{
	...
	int rc;
	struct rt_item nitem = {.saddr = 0, .ifindex = 0, .eth_source = {0}, .eth_dest = {0}};
	...
	if (nh_type == bpf_htons(ETH_P_IP)) { 
		...
		if (mac_zero(nitem.eth_dest)) {
			...
			action = bpf_redirect(nitem.ifindex, 0);
